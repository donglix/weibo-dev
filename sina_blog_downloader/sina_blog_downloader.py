...
def download(thread_n,thread_all,input_file,output_folder,concurrent_thread,num_of_subfolder,skip_count):
    #save log file
    ...
    for line in log_file:
            ...
            try:
                ...
                if(not os.path.exists(sub_folder)):         
                    try:
                        ...
                        print e
                        pass
                file_name=string.strip("%s/%s%s"% (sub_folder,line,".json"))
		...
		for result in con_dict["result"]:
			str_con=str(result["uid"])+" "+str(result["mblogid"])+"\n"+result["content"]["text"]+"\n"
			output.write(str_con.encode("UTF-8"))
                output.close()        
...
for i in range(0,thread_count):
      print i
      t=threading.Thread(target=download,args=(i,thread_count,input_file,output_folder,concurrent_thread,sub_folder,skip_count))              
      threads.append(t)
for i in range(0,thread_count):
      threads[i].start()
for i in range(0,thread_count):
